...
def sets_operation(op,dynamic_set, static_set):
    result = set()
    if op=='d&s':
        for d_endpoint in dynamic_set:
            for s_endpoint in static_set:
                if same_urls(d_endpoint, s_endpoint):
                    result.add(s_endpoint)
                    break
    elif op=='d-s':
        result=dynamic_set.copy()
        for d_endpoint in dynamic_set:
            for s_endpoint in static_set:
                if same_urls(d_endpoint, s_endpoint):
                    result.remove(d_endpoint)
                    break
    elif op=='s-d':
...
for config in configurations:
    ...
    for endpoint in dynamic_dependencies_per_endpoint:
        ...
        if static_endpoint:
            ...
            compare_per_endpoint[endpoint]={'static_endpoint': static_endpoint,'numberOfDynamicDeps': len(dynamic_dependencies_set), 'numberOfStaticDeps': len(static_dependencies_set), 'intersectionOfDepsLength': len(sets_operation('d&s',dynamic_dependencies_set,static_dependencies_set)), 'dynamicAndNotStatic': len(sets_operation('d-s',dynamic_dependencies_set,static_dependencies_set)), 'staticAndNotDynamic': len(sets_operation('s-d',dynamic_dependencies_set,static_dependencies_set))}
    ...
    x_axis.append('edgesPerNodes x 10')
    dynamic_statistics.append(10*dynamic_statistics[0]/dynamic_statistics[5])
    static_statistics.append(10*static_statistics[0]/static_statistics[5])
